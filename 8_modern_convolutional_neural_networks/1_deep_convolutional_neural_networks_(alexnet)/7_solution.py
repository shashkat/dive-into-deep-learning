...
import wandb
run = wandb.init(project="d2l_8p7_lenet_modifications", name = 'lenet_base') # assigning this is important in terminal ipython as else an error appears as it tries to print whatever is returned wand.init() which is not printable in terminal ipython
...
class LeNet_v1(nn.Module, HyperParameters):
...
class LeNet_v2(nn.Module, HyperParameters):
	def __init__(self, lr=0.1, num_classes=10):
		super().__init__()
		self.save_hyperparameters()
		self.net = nn.Sequential(
...
class LeNet_v3(nn.Module, HyperParameters):
	def __init__(self, lr=0.1, num_classes=10):
		super().__init__()
		self.save_hyperparameters()
		self.net = nn.Sequential(
...
# run 1
model1 = LeNet_v1(lr=0.01)
trainer.fit(model1, data)

wandb.finish()
run = wandb.init(project="d2l_8p7_lenet_modifications", name = 'lenet_modification')

# run 2
model2 = LeNet_v2(lr=0.01)
trainer.fit(model2, data)

wandb.finish()
run = wandb.init(project="d2l_8p7_lenet_modifications", name = 'lenet_mod_without_dropout')

# run 3
model3 = LeNet_v3(lr=0.01)
trainer.fit(model3, data)

# CONCLUSION:
# SO JUST CHANGING SIGMOIDS TO RELU IMPROVED THE MODEL SO MUCH. INITIALLY, I MADE BOTH THE 
# CHANGES AT ONCE - CHANGE SIGMOID TO RELU AND ADD DROPOUT. THEN, IN THE V3 RUN, I TRIED TO 
# JUST KEEP THE CHANGE OF SIGMOID TO RELU, BUT REMOVED THE ADDED DROPOUT LAYER. THIS ACTUALLY 
# LED TO EVEN BETTER RESULTS THAN V2, BUT THERE WAS SOME INDICATION OF POSSIBLE OVERFITTING IN 
# EPOCH 5. HENCE, I WAS ABLE TO APPRECIATE THAT DROPOUT HELPS IN REDUCING OVERFITTING. 
