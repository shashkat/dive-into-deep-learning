...
import wandb
run = wandb.init(project="d2l_8p4_alexnet_and_lenet_comparison") # assigning this is important in terminal ipython as else an error appears as it tries to print whatever is returned wand.init() which is not printable in terminal ipython
...
class AlexNet(nn.Module, HyperParameters):
	...
	def training_step(self, batch):
		...
		wandb.log({'training_loss_alexnet': l})
		print(f'loss = {l}')
		return l

	def validation_step(self, batch):
		l = self.loss(self(*batch[:-1]), batch[-1])
		wandb.log({'validation_loss_alexnet': l})
...
class LeNet(nn.Module, HyperParameters):
	def __init__(self, lr=0.1, num_classes=10):
		super().__init__()
		self.save_hyperparameters()
		self.net = nn.Sequential(
	...
	def training_step(self, batch):
		l = self.loss(self(*batch[:-1]), batch[-1])
		wandb.log({'training_loss_lenet:': l})
		print(f'loss = {l}')
		return l

	def validation_step(self, batch):
		l = self.loss(self(*batch[:-1]), batch[-1])
		wandb.log({'validation_loss_lenet:': l})
...
data1 = FashionMNIST(batch_size=128)
model1 = LeNet(lr=0.01)
trainer1 = Trainer(max_epochs=30, num_gpus=1)
trainer1.fit(model1, data1)

data2 = FashionMNIST(batch_size=128, resize=(224, 224))
model2 = AlexNet(lr=0.01)
trainer2 = Trainer(max_epochs=10, num_gpus=1)
trainer2.fit(model2, data2)

# CONCLUSION:
# WITH AROUND 2 EPOCHS ONLY, THE LOSS OF ALEXNET GOES TO AROUND 0.5. HOWEVER, FOR LENET, 
# EVEN WITH 30 EPOCHS, WHICH TAKE AROUND SIMILAR TIME AS 2 EPOCHS FOR ALEXNET, THE LOSS 
# IS STILL AROUND 2.3. THIS COULD BE ATTRIBUTED TO THE SMALL/SHALLOW MODEL THAT LENET IS. 
# IT IS NOT ABLE TO FIND PATTERNS IN FASHIONMNIST DATASET.
