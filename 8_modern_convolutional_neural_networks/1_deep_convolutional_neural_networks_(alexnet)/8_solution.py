...
class AlexNet_simple1(nn.Module, HyperParameters):
...
class AlexNet_simple2(nn.Module, HyperParameters):
	def __init__(self, lr=0.1, num_classes=10):
		super().__init__()
		self.save_hyperparameters()
		self.net = nn.Sequential(
			...
			# nn.Dropout(p=0.5), # just remove the dropout layer
			nn.LazyLinear(num_classes))
		self.net.apply(init_cnn)
		self.loss = nn.CrossEntropyLoss()
...
run = wandb.init(project="d2l_8p1_q8_alexnet_break", name = 'alexnet_simple') # assigning this is important in terminal ipython as else an error appears as it tries to print whatever is returned wand.init() which is not printable in terminal ipython

# run 1
model = AlexNet_simple1(lr=0.01)
trainer.fit(model, data)

wandb.finish()
run = wandb.init(project="d2l_8p1_q8_alexnet_break", name = 'alexnet_no_dropout')

# run 2
model = AlexNet_simple2(lr=0.01)
trainer.fit(model, data)

wandb.finish()
run = wandb.init(project="d2l_8p1_q8_alexnet_break", name = 'alexnet_no_dropout_10eps')
...
# CONCLUSION:
# SO I JUST REMOVED THE DROPOUT LAYER IN ORDER TO TRY CAUSE OVERFITTING, BUT IT DIDN'T IN 
# THIS CASE. I CAN COME UP WITH OTHER WAYS TO ALSO CAUSE OVERFITTING OR BREAK THE ALEXNET
# MODEL BUT FOR NOW, I AM STOPPING HERE.
