...
# As expected, the learning happens only for the state just before the goal state. Before that,
# the states don't have incentive to take action towards the before-goal state, as the 0 gamma
# makes the action-value function for those actions to be 0.

gamma = 0.5  # Discount factor
env_info = frozen_lake(seed, map_name)
q_learning(env_info=env_info, gamma=gamma, num_iters=num_iters, alpha=alpha, epsilon=epsilon, size = map_name)
# The learning happens in this case. A relatively low gamma value means that the model is 
# incentivized to take the shortest path a lot more. However, the way we have framed our policy 
# function, the action when 'exploiting' (opposite of exploring) is always of choosing the action
# with highest action-value function from the current state. Hence, how much higher the value of 
# max-valued action is compared to other possible actions, doesn't play a role.

gamma = 1  # Discount factor
env_info = frozen_lake(seed, map_name)
q_learning(env_info=env_info, gamma=gamma, num_iters=num_iters, alpha=alpha, epsilon=epsilon, size = map_name)
# Again, not much learning happens as the model is not incentivized to keep the path as short as 
# possible, and the action of going outside the board also yields the same as going in the correct 
# direction.
