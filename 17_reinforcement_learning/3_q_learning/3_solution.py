...
epsilon = 0  # Epsilon in epsilion gready algorithm
...
# Learning doesn't happen because the model doesn't explore any new actions only. Hence, it is 
# not able to gather data about which actions from which states yield possibly better rewards.
# Infact, it is not able to visit new states only, as it is not exploring, hence it keeps on 
# making the same mistakes. Due to a default max_episode_steps of our env to be 100, we move 
# out of the loop, otherwise any iteration would never end, as the state is start, and action 
# is left, which brings us back to starting state and nothing new ever happens.

epsilon = 0.5  # Epsilon in epsilion gready algorithm
num_iters = 2000  # Number of iterations
env_info = frozen_lake(seed, map_name)
q_learning(env_info=env_info, gamma=gamma, num_iters=num_iters, alpha=alpha, epsilon=epsilon, size = map_name)
# learning happens, but only after we run for much more iterations. This is because epsilon = 0.5 
# doesn't provide enough of exploration to our model initially to be able to reach the goal quickly 
# for the first time. Before that happening, nothing ever changes in the model, as the reward is 
# only given once the goal is reached. 

epsilon = 1  # Epsilon in epsilion gready algorithm
num_iters = 256  # Number of iterations
env_info = frozen_lake(seed, map_name)
q_learning(env_info=env_info, gamma=gamma, num_iters=num_iters, alpha=alpha, epsilon=epsilon, size = map_name)
# such a high epsilon is definitely great for learning the optimal actions from the different 
# states. However, when actually applying, i.e. trying to reach the goal from the start to apply 
# the model, it would be better to reduce the exploration, because otherwise the model will still 
# keep on exploring then too. Another disadvantage of having epsilon = 1 is that the model will 
# not try to focus on the already known good paths, and maybe exploring them in detail where there
# is uncertainty. It will just keep on exploring everything without making any use of the learned
# 'good patterns'. This is difficult to see in this example as our states are very few, but if they
# were much more, we would observe that our model doesn't go into exploring the already good 
# options into more detail to maybe make them best, but keeps on exploring among the good and bad 
# options only, i.e doesn't give enough time to exploring the good options in more detail.
